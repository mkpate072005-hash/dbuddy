...
import { Database, Search, Download, Trash2, Plus, Calendar } from 'lucide-react';
...
const DB_STYLES: Record<string, { badge: string; dot: string; glow: string }> = {
  PostgreSQL: {
    ...
    dot: 'bg-cyan-400 shadow-[0_0_6px_rgba(0,245,255,0.6)]',
    glow: 'hover:border-cyan-400/40 hover:shadow-[0_0_30px_rgba(0,245,255,0.08)]'
  },
  MySQL: {
    badge: 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20',
    dot: 'bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.6)]',
    glow: 'hover:border-emerald-400/40 hover:shadow-[0_0_30px_rgba(52,211,153,0.08)]'
  },
  MongoDB: {
    badge: 'text-orange-400 bg-orange-400/10 border-orange-400/20',
    dot: 'bg-orange-400 shadow-[0_0_6px_rgba(251,146,60,0.6)]',
    glow: 'hover:border-orange-400/40 hover:shadow-[0_0_30px_rgba(251,146,60,0.08)]'
  },
  SQLite: {
    badge: 'text-blue-400 bg-blue-400/10 border-blue-400/20',
    dot: 'bg-blue-400 shadow-[0_0_6px_rgba(96,165,250,0.6)]',
    glow: 'hover:border-blue-400/40 hover:shadow-[0_0_30px_rgba(96,165,250,0.08)]'
  },
};
          ...
          <AnimatePresence>
            {databases.map((db, i) => {
              ...
              return (
                ...
                >
                  <div className={`
                    bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-2xl p-5
                  ...
                  `}>
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-600 mb-5">
                      ...
                      {tables.length > 0 && (
                        <span className="flex items-center gap-1.5">
                          <Database size={11} />
                          {tables.length} {db.type === 'MongoDB' ? 'collections' : 'tables'}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="w-9 h-9 flex items-center justify-center rounded-full border border-red-500/20 text-red-500/50 hover:text-red-400 hover:border-red-500/50 hover:bg-red-500/10 transition-all"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
          ...
          >
            ...
            >
              <GlassCard className="p-6 max-w-sm w-full">
                ...
                <p className="text-gray-400 text-sm mb-6">This action cannot be undone. All schema data will be permanently deleted.</p>
                <div className="flex gap-3">
                  <GlowButton variant="ghost" size="sm" className="flex-1" onClick={() => setConfirmDelete(null)}>
                    Cancel
                  </GlowButton>
                  <GlowButton
                    variant="danger"
